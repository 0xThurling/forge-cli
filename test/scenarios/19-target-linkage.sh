...
scenario_19_target_linkage() {
  ...
  make_library() { # <dir> <name> <linkage> <install_headers>
    local dir="$1" name="$2" linkage="$3" headers="$4"
...
LUA
  ...
  # Static library with installed headers.
  local s="$base/static"
  make_library "$s" lib_static static true
